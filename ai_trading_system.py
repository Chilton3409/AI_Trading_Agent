...
class AITradingSystem():
    ...
    async def market_surveillance(self, movers:list):
        try:
             if movers:
                for mover in movers:
                    price = Decimal(mover.price)
                    if price > Decimal('3.00') and price < Decimal('10.00'):
    ...
    async def sell_logic(self):
        ...
        try:
            ...
            daytrade_count = await self.daytrade_count(account)
            print(daytrade_count)
            for pos in positions:
                current_price = Decimal(pos.current_price)
                average_entry = Decimal(pos.avg_entry_price)
                pnl_percent = await self.calculate_pnl_percent(current_price=current_price, average_entry=average_entry)
...
async def main():
    ...
    while True:
        try:
            # 1. You must AWAIT the clock and refresh it every loop
            clock = ai_trading_system.client.client.get_clock()
            
            if clock.is_open:
                # 2. Refresh account data INSIDE the loop so buying power updates
                ...
                print(f"Market is open. Buying Power: {buying_power}")
                await ai_trading_system.buy_logic(buying_power=buying_power, day_trades=day_trades)
                await ai_trading_system.sell_logic()
                
                # 3. Sleep to prevent hitting Alpaca's 200 requests/min limit
                await asyncio.sleep(60) 
            else:
                # Today is Jan 17 (Saturday), so this will trigger.
                # Monday Jan 19 is MLK Day, so it will also trigger then.
                print(f"Market is closed. Next open: {clock.next_open}. Checking again in 15 mins...")
