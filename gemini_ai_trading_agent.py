...
class AITradingSystem():
    ...
    async def get_market_movers(self, top):
        ...
        try:
            ...
            market_movers = await self.client.get_market_movers(top=50)
            if market_movers:
    ...
    async def sentiment_analysis(self, news, candles):
        ...
        """
        sentiment = await self.client.ai_api.create_gemini_response(gemini_ai_text=f"Conduct a sentiment analysis on this news: {news} with these candles: {candles}")
        return sentiment
    ...
    async def ai_top_picks(self, ) -> list:
        ...
        """
        picks = await self.client.ai_api.create_gemini_response(gemini_ai_text=f"based on the stocks you have reviewed, what are your top picks? Return a list of top picks in the format ['pick1', 'pick2', ...]. Only the list so I can use it in a function")
    ...
    async def market_surveillance(self, movers:list):
        try:
            if movers:
                for mover in movers:
                    print(mover.symbol)
                    if mover.price >= Decimal('1.50') and mover.price <= Decimal('20.00'):
                ...
                if buying_power >= self.hold_amount:
                    ...
                    # i need to filter the list and extract the tickers
                        for ticker in ticker_list:
                            #create buy order
                            print(f"the ai wants to purchase: {ticker}")
    ...
    async def ai_portfolio_analysis(self, positions):
        """
        ...
        try:
            ...
            if positions:
                ...
                analysis = await self.client.ai_api.create_gemini_response(gemini_ai_text=f"analyze my current positions and determine which positons to sell: {positions}. Please only return a python list.")
                if analysis:
    ...
    async def sell_logic(self):
        try:
            ...
            ai_recommendations = await self.ai_portfolio_analysis(positions=positions)
...
async def main():
    ...
    buying_power = await ai_trading_system.current_buying_power(account)
    day_trades = await ai_trading_system.daytrade_count(account)
    await ai_trading_system.buy_logic(buying_power=buying_power, day_trades=day_trades)
