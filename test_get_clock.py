...
class AITradingSystem():
    ...
    async def get_market_clock(self):
        """ Retrieve the current market clock status. """
        try:
            clock = await self.client.client.get_clock()
            return clock
        except Exception as e:
            logging.exception(msg=f"Error fetching market clock: {e}")
            return None
    async def cycle():
        """
